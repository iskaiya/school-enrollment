@extends('common.main')
@section ('title', 'Pending Status')
@section('content')


<div class="container">
    <h1 class="title-page">Pending ({{$students->count()}})</h1>
        <div class="card text-center">
        <div class="card-header d-flex justify-content-end align-items-center card-header-summary">
                    <form method = "GET" action = "{{route('admin.pendingPage')}}" class="search d-flex" id="searchBar">
        ...
        <tbody>
            @foreach($students as $student)
                <tr>
                    ...
                    <td>
                            <form action = "{{route('admin.approved', $student->user_id) }}" method ='POST' style="display: inline-block;" >
                            @csrf
                            <button type = "submit" class="icon-button">
                                <i class="bi bi-check-lg text-success"></i>
                            </button>
                            </form>
                            <form action = "{{route('admin.reject', $student->user_id) }}" method ='POST' style="display: inline-block;">
                                @csrf
                                <button type = "submit" class="icon-button">
                        ...
                        <button type="button" class="icon-button" data-bs-toggle="modal" data-bs-target="#staticBackdrop2{{ $student->user_id}}">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn-close2" data-bs-dismiss="modal">Close</button>
                            </div>
                            </div>
                        </div>
                        </div>
